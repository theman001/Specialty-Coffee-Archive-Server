...
(function () {
  ...
  const TABS = [
    ...
    { id: 'aroma',     label: '향' },
    { id: 'flavor',    label: '맛' },
    { id: 'intensity', label: '강도·질감' },
    { id: 'overall',   label: '총평' },
  ];

  const AROMA_HOT = ['과일향','베리류','감귤류','열대과일','꽃향','장미','자스민','초콜릿','캐러멜','견과류','구수함','흙냄새','나무향','스파이시'];
  const AROMA_ICE = ['시원한 과일향','민트','풀냄새','상큼한 감귤','베리류','초콜릿','꽃향','달콤함','가벼운 향'];
  const FLAVOR_HOT = ['체리','딸기','블루베리','레몬','오렌지','사과','복숭아','열대과일','다크초콜릿','밀크초콜릿','캐러멜','흑설탕','아몬드','헤이즐넛','홍차','자스민','건포도','시럽'];
  const FLAVOR_ICE = ['레몬','라임','자몽','청포도','수박','딸기','블루베리','복숭아','사이다 청량감','초콜릿','캐러멜','민트'];
  const SCALES_HOT = [['acidity','산미'],['sweetness','단맛'],['bitterness','쓴맛'],['body_score','바디'],['balance','밸런스'],['aftertaste','여운']];
  const SCALES_ICE = [['acidity','산미'],['sweetness','단맛'],['bitterness','쓴맛'],['body_score','바디'],['refreshing','청량감'],['aftertaste','여운']];
  ...
  function savePanelState() {
    ...
    _container.querySelectorAll('[id^="tf-"]').forEach(el => {
  ...
  function restoreAllState() {
    ...
    Object.entries(_vals).forEach(([key, val]) => {
      const el = _container.querySelector('#tf-' + key);
      if (el) el.value = val;
    });
    ...
    if (_star > 0) {
      ...
      const h = _container.querySelector('#tf-star-hint');
  ...
  function panelInfo() {
    ...
    return `<div class="space-y-3">
      <div class="grid grid-cols-2 gap-2">
        <div><label class="${lbl}">날짜</label><input type="date" id="tf-date" class="${ipt}"></div>
        <div><label class="${lbl}">카페/구입처</label><input type="text" id="tf-cafe" class="${ipt}" placeholder="블루보틀 성수"></div>
      </div>
      <div class="grid grid-cols-2 gap-2">
        <div><label class="${lbl}">원두 이름 <span class="text-red-400 normal-case font-normal">*</span></label>
          <input type="text" id="tf-bean" class="${ipt}" placeholder="예가체프 G1"></div>
        <div><label class="${lbl}">종류</label>
          <select id="tf-bean-type" class="${ipt}">
            <option value="">선택</option><option>싱글오리진</option><option>블렌드</option><option>모름</option>
          </select></div>
      </div>
      <div class="grid grid-cols-3 gap-2">
        <div><label class="${lbl}">원산지</label><input type="text" id="tf-origin" class="${ipt}" placeholder="에티오피아"></div>
        <div><label class="${lbl}">프로세싱</label>
          <select id="tf-process" class="${ipt}">
        ...
        <div><label class="${lbl}">로스팅</label>
          <select id="tf-roast" class="${ipt}">
            <option value="">모름</option><option>라이트</option><option>미디엄라이트</option><option>미디엄</option><option>미디엄다크</option><option>다크</option>
          </select></div>
      </div>
      <div class="border-t border-slate-100 dark:border-coffee-border pt-3">
        <p class="text-[10px] font-bold text-slate-400 dark:text-coffee-muted uppercase tracking-wider mb-2">⚗️ 드립 레시피 (선택)</p>
        <div class="grid grid-cols-3 gap-2">
          <div><label class="${lbl}">원두(g)</label><input type="text" id="tf-dose" class="${ipt}" placeholder="15"></div>
          <div><label class="${lbl}">물(ml)</label><input type="text" id="tf-water" class="${ipt}" placeholder="250"></div>
          <div><label class="${lbl}" id="tf-temp-lbl">${tempLbl}</label><input type="text" id="tf-temp" class="${ipt}" placeholder="92"></div>
        </div>
        <div class="grid grid-cols-2 gap-2 mt-2">
          <div><label class="${lbl}">그라인더/분쇄도</label><input type="text" id="tf-grind" class="${ipt}" placeholder="코만단테 #24"></div>
          <div><label class="${lbl}">추출시간</label><input type="text" id="tf-time" class="${ipt}" placeholder="2분 30초"></div>
        </div>
        <div class="mt-2"><label class="${lbl}">드리퍼/필터</label><input type="text" id="tf-dripper" class="${ipt}" placeholder="V60 01, 오리가미"></div>
        <div id="tf-ice-extra" class="mt-2 ${iceHide}">
          <div class="grid grid-cols-2 gap-2">
            <div><label class="${lbl}">얼음(g)</label><input type="text" id="tf-ice" class="${ipt}" placeholder="150"></div>
            <div><label class="${lbl}">추출방식</label>
              <select id="tf-ice-type" class="${ipt}">
  ...
  function panelAroma() {
    ...
    return `<div class="space-y-3">
      ...
      <div><label class="${lbl}">내 말로 묘사하기</label>
        <textarea id="tf-aroma-note" rows="3" class="${ipt} resize-none mt-0.5" placeholder="예) 뭔가 달콤하고 가벼운 냄새, 꽃 같기도 함"></textarea>
        <p class="text-[10px] text-slate-400 dark:text-coffee-muted mt-1">정확한 단어가 없어도 괜찮아요. 느낌대로 쓰는 게 훈련입니다.</p>
      </div>
      <div id="tf-aroma-hot-sec" class="${hotHide}">
        <p class="text-[10px] font-bold text-slate-400 dark:text-coffee-muted uppercase tracking-wider mb-2">🌡️ 온도별 향 변화</p>
        <div><label class="${lbl}">뜨거울 때 (80°C~)</label><textarea id="tf-aroma-h" rows="2" class="${ipt} resize-none mt-0.5" placeholder="진하고 스모키한 느낌"></textarea></div>
        <div class="mt-2"><label class="${lbl}">따뜻할 때 (~60°C)</label><textarea id="tf-aroma-w" rows="2" class="${ipt} resize-none mt-0.5" placeholder="달콤한 향이 올라오기 시작"></textarea></div>
        <div class="mt-2"><label class="${lbl}">식었을 때 (~40°C)</label><textarea id="tf-aroma-c" rows="2" class="${ipt} resize-none mt-0.5" placeholder="과일향이 가장 뚜렷해짐"></textarea></div>
      </div>
    </div>`;
  }
  ...
  function panelFlavor() {
    ...
    return `<div class="space-y-3">
      ...
      <div><label class="${lbl}">내 말로 묘사하기</label>
        <textarea id="tf-flavor-note" rows="3" class="${ipt} resize-none mt-0.5" placeholder="예) 처음엔 새콤, 넘길 때 달달함이 남음"></textarea>
      </div>
      <div id="tf-ice-flavor" class="${iceHide}">
        <label class="${lbl} mt-2">핫과 비교해 달라진 점</label>
        <textarea id="tf-ice-diff" rows="2" class="${ipt} resize-none mt-0.5" placeholder="산미가 훨씬 두드러지고 바디감이 가벼워진 것 같음"></textarea>
    ...
    return `<div>
      ...
      <div class="border-t border-slate-100 dark:border-coffee-border pt-3 mt-2">
        ...
        <div class="mt-2">
          ...
          <textarea id="tf-finish-note" rows="2" class="${ipt} resize-none mt-0.5" placeholder="예) 쓴 맛이 오래 남고 혀가 약간 뻣뻣한 느낌"></textarea>
        </div>
      </div>
    </div>`;
  }
  ...
  function panelOverall() {
    ...
    return `<div class="space-y-3">
      <div>
        ...
        <p class="text-[10px] text-slate-400 dark:text-coffee-muted mt-1" id="tf-star-hint">${hintTxt}</p>
      </div>
      <div><label class="${lbl}">한 줄 총평</label>
        <input type="text" id="tf-summary" class="${ipt} mt-0.5" placeholder="산미가 강한데 은근히 당기는 커피">
      </div>
      <div>
      ...
      <div class="border-t border-slate-100 dark:border-coffee-border pt-3">
        <p class="text-[10px] font-bold text-slate-400 dark:text-coffee-muted uppercase tracking-wider mb-2">📝 훈련 메모</p>
        <div><label class="${lbl}">오늘 어려웠던 것</label>
          <textarea id="tf-hard" rows="2" class="${ipt} resize-none mt-0.5" placeholder="예) 산미인지 떫음인지 구분이 잘 안 됐음"></textarea>
        </div>
        <div class="mt-2"><label class="${lbl}">다음 번에 집중할 것</label>
          <textarea id="tf-next" rows="2" class="${ipt} resize-none mt-0.5" placeholder="예) 향 맡을 때 온도 변화에 따라 더 꼼꼼히 맡아볼 것"></textarea>
        </div>
        <div class="mt-2"><label class="${lbl}">추가 메모</label>
          <textarea id="tf-note" rows="2" class="${ipt} resize-none mt-0.5" placeholder="자유롭게..."></textarea>
        </div>
      </div>
    </div>`;
  }
  ...
  function renderInto(containerId) {
    ...
    _container.innerHTML = `
      ...
      <div class="flex overflow-x-auto mb-3 border-b border-slate-200 dark:border-coffee-border">${tabBar}</div>
      <div id="tf-p-info">${panelInfo()}</div>
      <div id="tf-p-aroma" class="hidden">${panelAroma()}</div>
      <div id="tf-p-flavor" class="hidden">${panelFlavor()}</div>
      <div id="tf-p-intensity" class="hidden">${panelIntensity()}</div>
      <div id="tf-p-overall" class="hidden">${panelOverall()}</div>`;

    restoreAllState();
    bindAll();
    setActiveTab(_tab);

    const dateEl = _container.querySelector('#tf-date');
    if (dateEl && !dateEl.value) dateEl.value = new Date().toISOString().split('T')[0];
  }

  function setActiveTab(tabId) {
    TABS.forEach(t => {
      const p = _container.querySelector('#tf-p-' + t.id);
  ...
  function bindAll() {
    ...
    _container.querySelectorAll('.tf-mode').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        Object.entries(targets).forEach(([id, fn]) => {
          const el = _container.querySelector('#tf-p-' + id);
  ...
  function reset() {
    _mode = 'hot'; _tab = 'info'; _star = 0; _scale = {}; _vals = {}; _tagSels = {};
  ...
  window.TastingForm = { init: renderInto, getData, populate, reset };

  // Auto-init for admin pages
  const autoEl = document.getElementById('tastingFormBody');
  if (autoEl) renderInto('tastingFormBody');
})();
